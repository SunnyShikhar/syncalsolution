...
<html lang="en">
	...
	<!-- Header -->
		...
		<?php
					...
					"(synNum, serialNum, comment, calpm, lastupdatedFirst, lastupdatedLast) " .
						"VALUES (?, ?, ?, ?, ?, ?)";				
					// Prepare statement
					$query1stmt = $conn->prepare($query1);
					// Bind input parameters
					$query1stmt->bind_param("ssssss", $_POST['synNum'], $_POST['serialNum'], $_POST['comment'], $_POST['calpm'], $_POST['lastupdatedFN'], $_POST['lastupdatedLN']);			
					$query1stmt->execute();	
					$query1stmt->close();	
					...
					$query2stmt->bind_result($synNum, $serialNum, $comment, $calpm, $lastupdatedFirst, $lastupdatedLast);
		?>
